...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await edit_or_reply(event, "```Mohon Balas Ke Pesan Pengguna atau ketik .dgrup (ID/Username) Yang mau Anda deteksi```")
        return
    if input_str:
        try:
            uid = int(input_str)
    ...
    chat = "@tgscanrobot"
    event = await edit_or_reply(event, "`Sedang Mendeteksi...`")
